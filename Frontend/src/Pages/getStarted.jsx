...
const ParkEaseSecureRegistration = () => {
  const [step, setStep] = useState(0); // 0: role, 1-6: registration steps, 7: success
  const [selectedRole, setSelectedRole] = useState(null); // 'driver' or 'owner'
          ...
          {stepsArr.map((label, idx) => {
            ...
            return (
              <div key={idx} className="flex flex-col items-center mx-1">
                <div
                  className={`h-1.5 rounded-full transition-all duration-300 ${status === "done"
                      ? "w-7 bg-orange-500"
                      : status === "active"
                        ? "w-11 bg-orange-500"
                        : "w-7 bg-white/10"
          ...
          <div className="flex-1 flex items-center justify-center p-6">
            <div className="max-w-2xl w-full">
              ...
              <div className="grid md:grid-cols-2 gap-4 mb-8">
                {/* Driver Card */}
                <div
                  className={`p-6 rounded-2xl cursor-pointer transition-all relative border-2 ${selectedRole === "driver"
                ...
                >
                  <div
                    className={`absolute top-3 right-3 w-6 h-6 rounded-full bg-orange-500 flex items-center justify-center text-white text-xs font-bold transition-opacity ${selectedRole === "driver" ? "opacity-100" : "opacity-0"
                      }`}
                  >
                    ✓
                  </div>
                ...
                {/* Owner Card */}
                <div
                  className={`p-6 rounded-2xl cursor-pointer transition-all relative border-2 ${selectedRole === "owner"
                      ? "border-orange-500 bg-orange-500/5"
                      : "border-white/10 bg-gray-900/50 hover:border-orange-500/30"
                    }`}
                ...
                >
                  <div
                    className={`absolute top-3 right-3 w-6 h-6 rounded-full bg-orange-500 flex items-center justify-center text-white text-xs font-bold transition-opacity ${selectedRole === "owner" ? "opacity-100" : "opacity-0"
                      }`}
                  >
                    ✓
                  </div>
  ...
  // SecureID Registration Steps (1-6)
              ...
              <div>
                <div>
                  ...
                  <input
                    ...
                    value={formData.dob}
                    onChange={handleInputChange}
                    className="w-full bg-gray-900/60 border border-white/10 rounded-xl p-3 text-white focus:border-orange-500 focus:outline-none transition-colors [color-scheme:dark]"
                    max={new Date().toISOString().split('T')[0]}
                    placeholder="YYYY-MM-DD"
                  />
                  {!formData.dob && formData.dob !== "" && (
                    <p className="text-xs text-red-400 mt-1">Please select your date of birth</p>
                  )}
                </div>
              </div>
            </div>
...
export default ParkEaseSecureRegistration;